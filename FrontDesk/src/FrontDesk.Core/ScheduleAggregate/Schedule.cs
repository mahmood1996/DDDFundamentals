  ...
  public class Schedule : BaseEntity<Guid>, IAggregateRoot
  {
    ...
    private void MarkConflictingAppointments()
    {
      foreach (var appointment in _appointments)
      {
        ...
        if (HasSameRoomConflicting(appointment))
        {
          potentiallyConflictingAppointments = sameRoomDifferentAppointments(appointment);
        }

        // TODO: Add a rule to mark same doctor with overlapping appointments as conflicting
        if (HasSameDoctorConflicting(appointment))
        {
          potentiallyConflictingAppointments = sameDoctorDifferentAppointments(appointment);
        }
    ...
    private bool HasSameRoomConflicting(Appointment appointment)
    {
      return sameRoomDifferentAppointments(appointment).Any();
    }

    private List<Appointment> sameRoomDifferentAppointments(Appointment appointment)
    {
      return _appointments
        .Where(_appointment => _appointment != appointment)
        .Where(_appointment => _appointment.RoomId == appointment.RoomId)
        .Where(_appointment => _appointment.TimeRange.Overlaps(appointment.TimeRange))
        .ToList();
    }

    private bool HasSameDoctorConflicting(Appointment appointment)
    {
      return sameDoctorDifferentAppointments(appointment).Any();
    }

    private List<Appointment> sameDoctorDifferentAppointments(Appointment appointment)
    {
      return _appointments
                .Where(_appointment => _appointment != appointment)
                .Where(_appointment => _appointment.DoctorId == appointment.DoctorId)
                .Where(_appointment => _appointment.TimeRange.Overlaps(appointment.TimeRange))
                .ToList();
    }
